###########################################
###   LOESUNG: DATENVISUALISIERUNG IN R ###
###########################################
...
## Übung 2: Daten laden --------------------------------------------------------
# Als nächstes laden wir den Datensatz `audit`, den wir inzwischen schon gut kennen. Die Daten finden wir unter folgendem Link:
audit <- rio::import('https://raw.githubusercontent.com/CorrelAid/rlernen_umwelt/main/daten/bffp2019_audit_by_country_and_company.csv')
...
# ... gruppieren die Daten nach den herstellenden Firmen ...
  dplyr::group_by(parent_company) %>%                                              
  
# ... und berechnen die Gesamtanzahl an Plastikstücken pro Firma ('total_pieces = sum(n_pieces').
...
# Wir verschaffen uns einen Überblick über das Ergebnis
print(top10_parentcompany)
...
ggplot2::ggplot(data = top10_parentcompany, 
                aes(x = total_pieces, y = reorder(parent_company, total_pieces))) + 
  
# ... entscheiden uns für das Balkendiagramm ('geom_bar') als Darstellungsform und legen die Farbe fest.
  ggplot2::geom_bar(stat = "identity", fill = "#4E97AC") + 
  
# Anschließend setzen wir den Titel und die Achsenbeschriftungen ...
  ggplot2::labs(
