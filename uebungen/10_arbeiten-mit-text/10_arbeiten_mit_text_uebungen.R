...
# Bevor wir die Firmennamen bereinigen, müssen wir uns zunächst einen Überblick über die möglichen Versionen des Firmennamens verschaffen. 
...
data_raw %>%
  ...
  dplyr::filter(stringr::???(parent_company, '???'))
...
data_raw %>%
  ...
  dplyr::mutate(
    ...
    # Mit str_replace() ersetzen wir alle Varianten des Firmennamens, die 'Pepsi' oder 'pepsi' enthalten, durch den einheitlichen Namen 'PepsiCo'.
    parent_company = stringr::???(parent_company, '.*[Pp]epsi.*', '???')
  ) %>%
  
  # Nach der Bereinigung filtern wir nun die Daten, um nur die Zeilen zu behalten, in denen der neue Name 'PepsiCo' vorkommt.
  dplyr::filter(parent_company == "???") 
...
data_raw %>%
  ...
  dplyr::filter(stringr::str_detect(parent_company, '???'))
...
data_raw %>%
  ...
  dplyr::mutate(
    ...
    parent_company = stringr::str_replace(???, '.*???*', 'Kraft Foods')
  ) %>%
  
  # Nach der Bereinigung filtern wir nun die Daten, um nur die Zeilen zu behalten, in denen der neue Name 'Kraft Foods' vorkommt.
  dplyr::filter(parent_company == "???") # Hier zeigen wir nur die Einträge mit dem neuen Firmennamen an.
...
data_raw %>%
  ...
  {wordcloud::???(words = .$???,                                         # Die Firmennamen
...
# Versucht das Ergebnis in eigenen Worten zu beschreiben. Was fällt Euch auf? Was ist vielleicht unklar?
