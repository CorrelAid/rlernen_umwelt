...
top10_companies <- audit %>%
  ...
  # Dann berechnet Ihr die Gesamtsumme der gesammelten Plastikteile für jede Firma. Wichtig: Mit dem Argument "na.rm = TRUE" sorgt Ihr dafür, dass fehlende Werte (NA) ignoriert werden.
  ...
  # ... wobei Ihr Euch ausschließlich auf die obersten 10 Firmen beschränken wollt - dafür verwendet Ihr das Argument "slice(1:10)".
  dplyr::slice(1:10) %>%
...
audit_top10 <- audit %>%
  dplyr::filter(parent_company %in% top10_companies) %>%
...
pivot_audit <- audit_top10 %>% 
  ...
  dplyr::summarise(n_pieces = sum(n_pieces, na.rm = TRUE)) %>%
  
  # Nun wandelt Ihr die Daten in eine breite Form um, bei der jede "parent_company" zu einer Spalte wird.
  tidyr::pivot_wider(names_from = parent_company, values_from = n_pieces, values_fill = 0)

  # "names_from = parent_company" definiert, welche Werte als Spaltennamen verwendet werden.
  # "values_from = n_pieces" gibt an, welche Daten in die Zellen geschrieben werden sollen.
...
joined_full <- dplyr::full_join(audit, country_info, by = "country")
...
joined_left <- dplyr::left_join(audit, country_info, by = "country")
...
joined_inner <- dplyr::inner_join(audit, country_info, by = "country")

# Der resultierende Datensatz enthält nur die Länder, die in beiden Datensätzen vorkommen. 
# Alle anderen Länder werden ausgeschlossen.
