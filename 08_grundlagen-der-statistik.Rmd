...
quiz(caption = "Fragen zur Pressemitteilung",
  question("Wie haben sich die Investitonen in Maßnahmen zur Nutzung erneuerbarer Energien von 2011 bis 2021 entwickelt?",
    ...
    incorrect = "Leider falsch. Wir müssen in diesem Fall die hellblauen Balken betrachten - die Legende unten Links gibt an, welche Farben, in welchem Bereich die Investitionen getätigt wurden. Die y-Achse gibt in diesem Schaubild die Höhe der Investitionen an. Betrachten wir die hellblauen balken, fällt auf, dass diese über den Zeitverlauf größer bzw. höher werden. Sie geben die Entwicklung der Investitionen in Maßnahmen zur Nutzung erneuerbarer Energien an. Diese haben sich in den entsprechenden zehn Jahren sogar fast verdoppelt. Tipp: Behalte die Legenden bzw. Beschriftungen der Achsen einer Grafik im Blick, diese sind zur Interpretation der Daten essenziell.",
    allow_retry = TRUE,
...
quiz(caption = "Fragen zur Pressemitteilung",
  question("Betrachten wir zunächst einmal die Kosten der PKW-Instandhaltung. Welcher Bereich weist zwischen 2015 und 2021 insgesamt die größten Preispreissteigerung auf? (Grafik 2)?",
  ...
  question("Werfen wir nun noch einen Blick auf die Verbraucher:innenpreise für Bahn und Co. Welche Bereiche haben sich zugunsten der Verbraucher:innen entwickelt? (Grafik 2)",
    answer("Verbraucherpreisindex, insgesamt"),
    ...
    incorrect = "Leider falsch. Betrachte noch einmal die verschiedenen Linien: Im Vergleich zum Ausgangspunkt im Jahr 2015, der für alle Bereiche zwischen 95-100€ lag, haben sich die Preise deutlich verändert. Liegen die Endpunkte über dem Ausgangsniveau, haben sich die Preise erhöht - liegen die Endpunkte darunter, so haben sich die Preise reduziert.",
    allow_retry = TRUE,
    try_again_button = "Nochmal versuchen"
    )
)
...
quiz(caption = "Fragen zu tabellarischen Daten",
  ...
  question("Wie viele Beobachtungen hat der Datensatz aus der Abbildung?",
    ...
    answer("5"),
    correct = "Richtig. Anhand der Anzahl der Zeilen können wir feststellen, dass der Datensatz vier Beobachtungen hat, schließlich gibt es vier Zeilen mit befüllten Werten und wie wir wissen: Jede Zeile ist eine Beobachtung/Messung.",
...
**Median:** Entspricht dem “wahren” Mittelpunkt einer Variable (50 % aller Beobachtungen sind kleiner und 50 % aller Beobachtungen sind größer als der Median). Zur Bestimmung des Medians ordnet man man alle Beobachtungen in aufsteigender Reihenfolge und bestimmt, welche Beobachtung genau in der Mitte liegt.
...
quiz(caption = "Frage zu Lageparametern",
  ...
  question(sprintf("Gegeben sei die folgende Variable metrischen Messniveaus mit folgenden Ausprägungen: $x = [0, 1, 2, 4, 3, 1, 2]$. Wie lautet der Mittelwert?"),
    ...
    answer("3"),
    answer("4"),
    correct = sprintf("Richtig! Der Mittelwert ist das, was wir häufig als den 'Durchschnitt' bezeichnen. Zur Berechnung müssen zuerst alle Werte aufaddiert werden (= 13), um anschließend durch die Anzahl der Messwerte (= 7) geteilt zu werden. In diesem Fall beträgt der Mittelwert 1,86. Wie wir sehen, ist der Mittelwert kein Wert, der in der Messung selber vorkommt, sondern ein fiktiver Wert."),
    incorrect = "Leider falsch. Der Mittelwert ist das, was wir häufig als Durchschnitt bezeichnen. Zur Berechnung müssen zunächst alle Werte aufsummert und anschließend durch die Anzahl der Messwerte geteilt werden. Tipp: Der Mittelwert kommt nicht zwingend selbst in der Messung vor."
    allow_retry = TRUE,
    try_again_button = "Nochmal versuchen"
    ),
...
**Spannweite (auch: Range)**: Entspricht der Differenz zwischen dem kleinsten Wert (**Minimum**) und dem größten Wert (**Maximum**) einer Variable.
