...
### **Packages des Tidyverse**

Hier nochmal ein kurzer Überblick über alle Packages, die im `Tidyverse`enthalten sind und wofür wir diese unter anderem verwenden können:
...
Nachdem wir unsere Daten erfolgreich importiert haben, können wir einen ersten Blick in den Datensatz werfen: Sieht ganz schön unordentlich aus oder? Um ein bisschen Ordnung in das Chaos zu bringen, müssen wir die Daten bereinigen. Bevor wir aber wild drauf los coden, überlegen wir uns erst einmal, welche Datensätze am Ende des Bereinigungsprozesses existieren sollen und wie wir dieses Ziel erreichen.
*Hinweis: Unser Rohdatensatz enthält Daten aus den Jahren 2019 und 2020. Da wir uns an dieser Stelle erst einmal mit den grundlegenden Funktionen beschäftigen und keinen komplexen Zeitvergleich durchführen wollen, werden wir uns auf die Daten aus dem Jahr 2019 beschränken. Aufgrund der Covid-19-Pandemie stehen für das Jahr 2020 nämlich nur sehr wenige Daten zur Verfügung.*

Da wir uns einerseits für Daten (aus dem Jahr 2019) interessieren, die die #Breakfreefromplastic-Community betriffen, möchten wir einmal den Datensatz  **`community`** erstellen. Mithilfe dieser Daten wollen wir das Verhalten der Community in den verschiedenen Ländern vergleichen. Daher beobachten wir die einzelnen Länder, d.h. **jede Zeile steht für ein Land**. Als "communitybezogene" Daten identifizieren wir folgende Variablen: **Kontinent, Land, Countrycode, Anzahl Plastikteile, Events und Freiwillige**. Diese werden in unseren Spalten abgetragen, schließlich interessieren wir uns unter anderem Dafür, wie viele Events in den verschiedenen Ländern stattgefunden haben oder wie viele Plastikteile jeweils gesammelt werden konnten.

Andererseits interessieren wir uns auch für Daten (aus dem Jahr 2019), die die Firmen betreffen, die für den Plastikmüll verantwortlich sind und die wir somit einer Prüfung (engl. Audit) unterziehen. Daher erstellen wir einen zweiten Datensatz mit dem Namen **`audit`**. An dieser Stelle wird es ein klein wenig komplexer. Da wir uns neben der herstellenden Firma auch für die Art des Plastikmülls (Plastiktyp) und das Land interessieren, in dem der Müll gefunden wurde, beobachten wir verschiedene Dinge auf einmal: **Jede Zeile bildet also eine einzigartige Kombination aus Hersteller und Plastiktyp pro Land**. Als "auditbezogene" Daten identifizieren wir folgende Variablen: **Kontinent, Land, Countrycode, Hersteller, Anzahl Plastikstücke**. Diese werden in unseren Spalten abgetragen, da wir uns wie gesagt dafür interessieren, welche Art von Plastikmüll in welchem Land auftritt und welche Firma dafür verantwortlich ist.
...
Unser Ziel ist nun also klar definiert: Wir wollen zwei aufgeräumte Datensätze erstellen, die jeweils unterschiedliche Variablen betrachten. Nun müssen wir uns überlegen, wie wir dieses Ziel erreichen können. Zum einen wollen wir natürlich ausschließlich "vollständige" Daten betrachten. Fälle, in denen zum Beispiel kein Land angegeben ist ("EMPTY") sortieren wir aus - ebenso Beobachtungen aus dem Jahr 2020 (s. Hinweis). Da die Ländernamen auch nicht in jedem Fall gleich angegeben wurden (z.B. ECUADOR, Ecuador) - manche Namen müssen wir auch zunächst übersetzen (z.B. Cote D_ivoire). Abschließend müssen wir die "community- bzw- auditbezogenen Veriablen auswählen und den Datensatz entsprechend umformen. Wir entscheiden und also für Folgendes Vorgehen:
...
Um die Basics der Datenbereinigung kennenzulernen, werden wir in dieser Woche vor allem das **`dplyr`**-Package verwenden. Dieses ist speziell für die Datenmanipulation entwickelt worden und liefert uns eine Reihe an Möglichkeiten, wie wir unsere Daten verändern können. An dieser Stelle wollen wir uns einmal die **sechs Grundverben** dieses Packages anschauen.
Hier eine kurze Überischt über die Funktionen:
...
![Graphische Veranschaulichung der filter()-Funktion](https://github.com/CorrelAid/lernplattform/blob/zoe_datentransformation/abbildungen/06_datentransformation/filter.png?raw=true){#id .class width=60% height=60%}

Beginnen wir also einmal mit der Funktion **`dplyr::filter()`** mit der wir - wie der Name bereits vermuten lässt - **Zeilen bzw. Beobachtungen filtern** können. Wir betrachten also zunächst den gesamten Datensatz und legen dann mithilfe der Funktion die Zeilen aus, die wir behalten wolllen. Dazu müssen wir bestimmte Bedingung(en) definieren, die die entsprechenen Zeilen erfüllen müssen. Die Grammatik, um diese Bedingungen festzulegen, haben wir bereits in der Session "Einführung in (R)Studio" unter dem Namen **"Kontrollstrukturen"** kennengelernt - vielleicht erinnert Ihr Euch. Wir schauen uns das Ganze aber auch direkt einmal an einem Beispiel an:
Da wir für die Analyse des Datensatzes einen **Ländervergleich** planen, wollen wir alle Beobachtungen aussortieren, für die kein Land hinterlegt wurde. Wenn kein Land hinterlegt ist, ist die Zeile `country` leer. Dafür formulieren wir folgende Bedingung: `country != "EMPTY"`. In manchen Fällen wurden außerdem die Firmen, die für den Plasikmüll verantwortlich sind "aufsummert" bzw. vermischt. In diesen Fällen wird in der Zeile `parent_company`der Wert "Grand Total" angegeben. Um diese Fälle ebenfalls auszusortieren formulieren wir die Bedingung `parent_company != "Grand Total"`. Dementsprechend sieht unser Code wie folgt aus:
...
Und jetzt Ihr: Unser Datensatz enthält Daten aus den Jahren 2019 und 2020. Einerseits sind wir nicht an einem Zeitvergleich interessiert und andererseits liegen uns aufgrund der Covid-Pandemie für das Jahr 202 schlicht zu wenige Daten vor. Daher möchte wir ausschließlich die Daten beibehalten, die aus dem Jahr 2019 stammen - versucht einmal den folgenden Code entsprechend zu ändern und fügt die entsprechende Bedingung hinzu:
...
![Graphische Veranschaulichung der mutate()-Funktion](https://github.com/CorrelAid/lernplattform/blob/zoe_datentransformation/abbildungen/06_datentransformation/mutate.png?raw=true){#id .class width=60% height=60%}

Nun wollen wir die die Variable `country` (zu dt. Land) bereinigen. Denn, wie so oft, herrscht Unordnung: Die Länderbezeichnungen für Großbritannien und die USA sind nicht standardisiert, die Groß- und Kleinschreibung ist vollkommen durcheinander und außerdem wollen wir die deutschen Bezeichnungen verwenden, sowie den Kontinent und Countrycode ergänzen. Klingt nach sehr viel Arbeit, aber zum Glück bietet uns das `dplyr`-Package mit der Funktion **`mutate`** die Möglichkeit, Berechnungen auf Spaltenebene durchzuführen. Oder einfach gesagt: Wie können unserem Datensatz neue Variablen hinzufügen oder bestehende Variablen bearbeiten. Die grundlegende Syntax folgt dabei der anderen Funktionen: `dplyr::mutate(name_der_neuen_spalte = bedingung1 + bedingung2 + ...)`.
...
In einem zweiten Schritt bedienen wir uns am **Package `stringr`**, das ebenfalls im Tidyverse enthalten ist und uns dabei hilft, Zeichenketten (engl. Strings) relativ einfach zu manipulieren und analysieren. Mit der darin enthaltenen Funktion **`str_to_title()`** wird nämlich ein "Titel" erzeugt, d.h. der Anfangsbuchstabe wird großgeschrieben. Damit wir nun auch die deutschen Bezeichnungen verwenden können, können wir das **Package `countrycode`** verwenden. Damit können wir die Namen übersetzen und kodieren

Wichtig: Welche Funktionen Ihr am Ende für Eure Bereinigungsschritte braucht, hängt auch immer vom Datensatz ab. Gerade am Anfang kann das etwas überischtlich werden, vor allem mit den verschiedenen Funktionen aus unterschiedlichen Packages. Keine Sorge, mit etwas mehr Erfahrung und einer Googlerecherche lassen sich für die meisten Herausforderungen wirklich tolle Hilfsmittel finden!*
...
![Graphische Veranschaulichung der group_by()-Funktion](https://github.com/CorrelAid/lernplattform/blob/zoe_datentransformation/abbildungen/06_datentransformation/groupby.png?raw=true){#id .class width=60% height=60%}
![Graphische Veranschaulichung der summarise()-Funktion](https://github.com/CorrelAid/lernplattform/blob/zoe_datentransformation/abbildungen/06_datentransformation/summarize.png?raw=true){#id .class width=60% height=60%}

Nach wie vor verfolgen wir mit unserer Datenbereinigung das Zeil der **Ländervergleiche**: Wie möchten herausfinden, in welchen Ländern am meisten Plastikmüll gesammelt wurde, die meisten Events durchgeführt wurden oder die meisten Freiwilligen aktiv sind. Bislang haben gibt es jedoch für die meisten der Länder verschiedene Beobachtungen, wodurch diese länderübergreifenden Vergleiche schlicht nicht möglich sind. Daher müssen wir innerhalb unseres `community` Datensatzes eine Struktur erschaffen, in dem eine Beobachtung pro Land vorhanden ist - wir müssen unsere bereinigten Daten also pro Land **gruppieren**. Dafür können wir aus dem `dplyr`auf zwei Funktionen zurückgreifen, die uns diesen Schritt ermöglichen und häufig miteinander kombiniert werden: Die Funktionen **`group_by()`** und **`summarise()`** auch (`summarize()`). Zusammen sind sie vor besonders nützlich für das Erstellen von Gruppenzusammenfassungen und die Berechnung von Statistiken für verschiedene Kategorien innerhalb eines Datensatzes. Die Funktion **`group_by()`** wird dabei  verwendet, um Daten nach einer oder mehreren Variablen zu gruppieren. Diese Gruppierung ermöglicht es, anschließende Berechnungen oder Zusammenfassungen auf Gruppenebene durchzuführen. Im Anschluss können mit der Funktion **`summarise()`**  (oder: `summarize()`) zusammenfassende Statistiken oder aggregierte Berechnungen für jede Gruppe im Datensatz erstellt werden.
Daher werden wir zuerst entsprechend der Kontinente und Länder gruppieren (`dplyr::group_by(continent, country, countrycode)`). Anschließend müssen wir ebenfalls entscheiden, wie die anderen Variablen zusammengefasst werden (`dplyr::summarise(…)`). In diesem Fall addieren wir die Plastikteile und wählen für die Anzahl der Events und Freiwilligen genau einen Wert aus. Diese sind im Originaldatensatz mehrfach vorhanden - etwas, das typischerweise beim Zusammenfügen von Datensätzen entsteht. Aber schauen wir uns das ganze Vorgehen doch einmal direkt in unserem R-Code an:
...
Kurzer Exkurs zwischendurch!
Bislang haben wir uns damit beschäftigt, wie wir einzelne Werte in Datensätzen filtern und  bereinigen. Mithilfe von R ist es allerdings auch möglich, die **gesamte Form** eines Datensatzes zu verändern.

Um einen solchen „Tide“ Datensatz zu erhalten, können wir auf Funktionen aus dem **`tidyr`**-Package zurückgreifen. Das Paket bietet verschiedene Werkzeuge zur Umformung und Bereinigung von Datensätzen, sodass sie in diesem klaren und strukturierten Format vorliegen. Unter anderem sind darin Funktionen zur Umstrukturierung der Daten enthalten (z. B. von einem breiten Format in ein langes Format oder umgekehrt). Wenn wir Daten für bestimmte Analysen oder Visualisierungen vorbereiten müssen, kann das durchaus sehr nützlich sein. Aus diesem Package hilft uns die Funktion **`pivot_longer`** dabei, um einen Datensatz von einem breiten Format in ein langes Format umzuwandeln. Was das bedeutet? In einem breiten Format sind Variablen in mehreren Spalten verteilt, während in einem langen Format jede Variable in einer Spalte steht und jede Zeile eine Beobachtung darstellt. Im Gegensatz dazu können wir die Funktion **`pivot_wider`** dafür verwenden, um einen Datensatz von einem langen Format in ein breites Format zu bringen. Dies kann notwendig oder von Vorteil sein, wenn wir Daten für bestimmte Berichte oder Analysen benötigen, die breitere Formate erfordern.
Wir können das direkt an unserem `audit`-Datensatz üben, den wir ebenfalls noch bereinigen müssen. Wir können beispielsweise die Plastikarten noch so transformieren, dass sie als eine Variable im Datensatz vorkommen:
...
Mit `dyplr::pivot_wider()` können wir den Datensatz auch wieder in das breite Format bringen. An dieser Stelle würden wir aber empfehlen, die Daten in der langen Form zu benutzen.
...
## **Die `select()`-Funktion**
![Graphische Veranschaulichung der select()-Funktion](https://github.com/CorrelAid/lernplattform/blob/zoe_datentransformation/abbildungen/06_datentransformation/select.png?raw=true){#id .class width=60% height=60%}

Bislang haben wir Zeilen bzw. Beobachtungen gefiltert, aber dabei soll es nicht bleiben. Denn das `dplyr` liefert uns glücklicherweise auch direkt eine Funktion, mit der wir auch **Spalten bzw. Variablen filtern** können: Die **`select()`**-Funktion. Sie ist also ein durchaus hilfreiches Werkzeug, weil sie es ermöglicht, den Datensatz auf die für die Analyse relevanten Spalten zu reduzieren. Wir können einerseits bestimmte Spalten auswählen, aber auch Spalten umbenennen und nach verschiedenen Kriterien auswählen, wie z. B. nach dem Namen, dem Typ oder der Position der Spalten. Wir können also entweder die Variablen auswählen, die im Datensatz enthalten bleiben sollen oder aber mit einem `-` die Variablen auswählen, die aus Eurem Datensatz gelöscht werden sollen. Der folgende Code löscht so zum Beispiel die Variable `grand_total` aus dem `data_raw`-Datensatz:
...
Schon ist ein großer Schritt getan und unsere beiden Datensätze sind bereinigt! Was meint Ihr: Sind die enthaltenen Werte richtig? Können wir diese Frage überhaupt einfach so beantworten? Schwierig, das auf den ersten Blick zusagen - aber wir können mit R eine Art **Qualitätsprüfung** durchführen. Schauen wir uns also für beide Datensätze einige Informationen an:
...
![Graphische Veranschaulichung der arrange()-Funktion](https://github.com/CorrelAid/lernplattform/blob/zoe_datentransformation/abbildungen/06_datentransformation/arrange.png?raw=true){#id .class width=60% height=60%}
...
Und dennoch sind wir noch nicht ganz fertig mit den `dplyr`-Package. Denn das hat einfach unglaublich viel zu bieten … - und eine Funktion, die wir Euch definitiv nicht vorenthalten möchten ist die **`arrange()`**-Funktion. Diese wird eher für die Transformation unserer Daten als für deren Bereinigung genutzt. Mit dieser Funktion kann man die Zeilen eines Datensatzes nach den Werten einer oder mehrerer Spalten zu sortieren bzw. **Ranglisten ausgeben**. Sie ist also eine große Hilfe, wenn wir Daten in einer bestimmten Reihenfolge analysieren oder präsentieren und z.B. Trends erkennen, Daten für Berichte vorbereiten oder Extremwerte identifizieren möchten. Wenn wir uns also für die „Top 10“ der Firmen interessieren, die am meisten Plastikmüll produzieren, können zunächst die Firmen auswählen und gruppieren, uns eine Zusammenfassung ausgeben lassen und dann auf die „Top 10“ beschränken. Anschließend können wir mit `dplyr::arrange()` die Werte einfach in absteigender Reihenfolge sortieren - das ist also die perfekte Zusammenfassung des `dplyr:
