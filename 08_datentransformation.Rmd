...
**Pivoting von Daten** (zu dt. schwenken) ist ein Begriff, der in der Datenanalyse verwendet wird, um eine Umstrukturierung oder Neuorganisation von Daten darzustellen, insbesondere bei Tabellen. Es ermöglicht es, Daten in verschiedenen Perspektiven oder Formen zu betrachten, was bei der Analyse hilfreich sein kann. Es gibt zwei Hauptarten des Pivoting:

1. Pivoting von **breit zu lang** (Long-Format)
2. Pivoting von **lang zu breit** (Wide-Format) 

Diese Form der Datentransformation ist nicht zwingend notwendig, aber manchmal durchaus sinnvoll, da manche Datensatze dadurch lesbarer werden. Deshalb schauen wir uns beide Formen des Pivoting einmal genauer an!
...
![*Graphische Veranschaulichung der pivot_wider()-Funktion*](https://github.com/CorrelAid/rlernen_umwelt/blob/main/abbildungen/08_datentransformation/pivot_wider().jpg){#id .class width=60% height=60%}<br>

Wenn Ihr mehr darüber wissen wollt, könnt Ihr noch einmal in die [R-Dokumentation des tidyverse](https://tidyr.tidyverse.org/articles/pivot.html#wider){target="_blank"} schauen. <br>
...
![*Graphische Veranschaulichung der pivot_longer()-Funktion*](https://github.com/CorrelAid/rlernen_umwelt/blob/main/abbildungen/08_datentransformation/pivot_longer().jpg){#id .class width=60% height=60%}<br>

Mehr dazu findet Ihr ebenfalls in der [R-Dokumentation zum tidyverse](https://tidyr.tidyverse.org/articles/pivot.html#longer){target="_blank"}. <br>
...
Es kann allerdings auch vorkommen, dass ein einzelner Datensatz uns nicht alle Informationen liefert, die wir für unsere Arbeite brauchen. In einem solchen Fall müssen wir **mehrere Datensätze zusammenfügen** - mit R kein Problem! <br>

## **Interaktive Übung**

Im Exkurs ["Eigene API-Anfragen" zur Session Datenimport](https://correlaid.shinyapps.io/05_1_datenimport-exkurs-api/){target="_blank"} haben wir bereits mit einem Datensatz der Weltbank zu Naturschutzgebieten (`wb_areas`) gearbeitet. In Form von verschidenen Indikatoren, liefert uns dieser Datensatz verschiedene umweltbezogene Daten, die für unsere Analyse oder die Arbeit von Breakfreefromplastic interessant sein könnten - z.B. bedrohte Fischarten, Anteil Naturschutzgebiete, usw. Wir werden nun wieder mit diesem Datensatz arbeiten, allerdings ist es auch kein Problem, wenn Ihr den Exkurs (noch) nicht angeschaut habt! Verschafft Euch doch zuerst nochmal einen kleinen Überblick über den Datensatz `wb_areas`: 
...
Die Indikatoren liefern uns verschiedene umweltbezogene Datenn: **ER.PTD.TOTL.ZS** gibt beispielsweise an, wie viel Prozent der Landes- und Wasserfläche aus Naturschutzgebieten besteht. Somit ist dieser Datensatz vor allem für die Planung von zukünftigen Aktivitäten und die Kommunikation mit Freiwilligen und/oder Fördernden wertvoll und daher auch für das Netzwerk #Breakfreefromplastics besonders spannend.

Den Datensatz `wb_areas` wollen wir nun mit dem `community`-Datensatz zusammenfügen. Damit das gelingen kann, brauchen wir eine **gemeinsame Variable**, die R dafür verwenden kann, um die Länder einander zuzuordnen. Diese gemeinsame Variable muss in beiden Datensätzen den **gleichen Namen** haben oder von uns zugeordnet werden. Außerdem muss die Kodierung (hier ISO2C) übereinstimmen. Da Ländernamen fehleranfällig sind, fokussieren wir uns auf die **Country Codes**:
...
Wir wissen nun, wie viele Länder in den jeweiligen Datensätzen vorhanden sind und können jetzt die beiden Datensätze zusammenfügen.Da in dem Datensatz `wb_areas` viel mehr Länder vorkommen, gibt es verschiedene Möglichkeiten:
