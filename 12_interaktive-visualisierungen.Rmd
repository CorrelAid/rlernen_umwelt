...
<div style="background-color: white; border: 3px solid #28a745; color: #dc3545; padding: 15px; margin: 10px 0;">
  <strong>Disclaimer:</strong> Liebe Teilnehmer:innen, die letzte Session des Kurses R Lernen ist sehr umfangreich und gerade zu Beginn sehr komplex. Lasst Euch davon nicht verunsichern - Ihr müsst die Inhalte nicht alle im Detail verstehen und könnt die Code-Snippets in dieser Woche auch einfach kopieren! Wir wollen Euch lediglich zeigen, was mit RStudio alles möglich ist. Wenn wir damit Euer Interesse geweckt haben und Ihr Lust auf ein eigenes Datenprojekt habt habt, könnt Ihr Euch jederzeit melden - wir freuen uns auf Eure Ideen!
</div>
...
### **3. App-Namen wählen**
Unsere App braucht einen informativen, einfachen Namen für Ihre App, z. B. „Meine-Erste-R-Shiny-App_2024“ (ohne Sonderzeichen oder Leerzeichen). Das Arbeitsverzeichnis, in dem die Datei gespeichert wird, wird ebenfalls angezeigt.
...
Durch den Klick auf "Create" wird unsere erste Shiny-App wird erstellt. Die Datei heißt standardmäßig „app.R“ und liegt im gewählten Verzeichnis.
...
Unsere erste Shiny-App ist nun bereit! Wir klicken auf „Open in Browser“ klicken und können nun schon ein bisschen mit den Interaktiven Funktionen experimentieren, z. B. der Anzahl der Bins im Histogramm.

----

# **Interaktive Übung**

Nachdem wir unsere erste Shiny-App erstellt haben, wollen wir nun eine interaktive Visualisierung für unseren eigenen Datensatz entwickeln. Wie bereits im Video gezeigt, besteht jede Shiny-App aus zwei Teilen: dem **Server** und der **UI (User Interface)**. Schauen wir uns an, was dahinter steckt:
...
<span style="color:red;"><strong>Achtung: Den Code müsst ihr lokal in RStudio ausführen. Kopiert die Code-Snippets Schritt für Schritt in eure lokale Datei und passt sie an.</strong></span>
...
In dieser Session wollen wir das **Balkendiagramm** aus der [Datenvisualisierung-Session](https://correlaid.shinyapps.io/09_datenvisualisierung/#Und_jetzt_Ihr_(optional){target="_blank"}) in eine interaktive App umwandeln. Das fertige Ergebnis könnt ihr euch [hier](https://correlaid.shinyapps.io/breakfreefromplastic/){target="_blank"} unter dem Tab **"Hersteller"** anschauen.

Hier ist der Code für das Balkendiagramm der Top 10 Firmen mit der höchsten Müllproduktion:

```{r plot_full_to_print, exercise=TRUE}
...
*Tipp 1: Kopiere den Code für das Balkendiagramm in die Grundstruktur der `app.R`-Datei, um die gesamte Logik an einem Ort zu bündeln.*

*Tipp 2: Lade in einer `Rmd`-Datei notwendige Packages, importiere und bereinige Daten und visualisiere sie. Den Code kannst du dann einfach in die Shiny-App übertragen.*
...
Eine Übersicht über alle möglichen Layout-Optionen in Shiny findet Ihr in der [Shiny Gallery von RStudio](https://shiny.posit.co/r/gallery/){target="_blank"} oder der offiziellen [Shiny Webiste](https://shiny.posit.co/r/articles/build/layout-guide/){target="_blank"} - klickt Euch dort gerne einmal durch!
...
|---------------------------------------|-------------------------------------|
| Freitext                             | `textInput("id", "Beschreibung")`  |
...
Nun haben wir festgelegt, dass wir in unserer Shiny-App eine Seitenleiste einbauen wollen, mit der die Nutzer:innen interagieren können. Nun müssen wir festlegen, dass über diese Seitenliste unsere Daten nach **Kontinenten gefiltert** werden können. Dazu gehen wir wieder zur `app.R` Datei.
...
*Hinweis: Der Code funktioniert nur lokal auf Euren Rechnern und wird auf der Lernplattform nicht angezeigt. Wir zeigen ihn hier, um den normalen Ablauf zu simulieren.*
...
<details>
<summary><h1><b>Exkurs: Hosten von Shiny-Apps</b></h1></summary>
  <br>
